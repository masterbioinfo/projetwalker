...
class Titration (object):
	...
	def plotChemShifts(self, residues=None, split = False):
		...
		def saveJobPickl(self, directoryIn):
			"""
			The function can save the job. She takes Titration object and diretoryIn dictionnary, 
			in arguments. She return a confirmation message.
		...
		def loadJobPickl(self, directoryIn):
			"""
			The function can load the last job save. She takes Titrarion object and directoryIn dictionnary in arguments. She
			return elementsLoads list. This list contains objects loaded from the save file.
			It's return error message if needed.
			"""
			...
			try :	
				with open("{0}saveJob.txt".format(directoryIn["pathIn"]), 'rb') as loadJobFile:
					...
					titrationObjects = my_depickler.load()
